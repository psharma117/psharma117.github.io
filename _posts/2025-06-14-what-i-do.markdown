...
My work lives in a small region of the atom, as pictured here
...
To describe the idea better, it's once again helpful to use an analogy.

Let's say you wanted to calculate the circumference of a circle in order to perfectly wrap a ribbon around a disk or something like that.
A quick Google search tells you that the circumference of a circle with a diameter $D$ is $\pi\times D$.
Now, $\pi$ is an infinite decimal equal to 3.1415927... blah blah blah. 
You can get an approximation to the 
